  ...
  try {
    ...
    if (!user) {
      try {
        ...
        console.error('[Auth] Error verifying cookie token:', cookieError);
      }
    }
    ...
    // Catch any unexpected errors and return unauthenticated context
    console.error('[Auth] Unexpected error in getAuthContext:', error);
    return { user: null, apiKey: null, scopes: [] };
  }
}
